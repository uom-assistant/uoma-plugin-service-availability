    ...
    <ul>
      <template v-for="(item, index) in list" :key="`all-${item.id}`">
        <li class="pt-1 flex items-center justify-between" :class="{ '-mb-0.5': index === list.length - 1 }">
          ...
          <span class="w-12px h-12px rounded-full mr-1 flex-center" :class="item.availability === 'ok' ? 'bg-green-500' : item.availability === 'issues' ? 'bg-yellow-500' : 'bg-red-500'">
            <span class="w-12px h-12px block rounded-full animate-ping animation-slow bg-opacity-40" :class="item.availability === 'ok' ? 'bg-green-500' : item.availability === 'issues' ? 'bg-yellow-500' : 'bg-red-500'" />
          </span>
        </li>
